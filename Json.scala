...
class Doc(val lines: List[String]) { 
  ...
  def hcatT(that: Doc) : Doc = { 
    var thisLines =  Doc.padEnd(this.lines, that.height, " " * width)
    thisLines = widen(thisLines)
    val thatLines = Doc.padEnd(that.lines, height, "") 
    Doc(thisLines.zip(thatLines).map(combine => combine._1 + combine._2))  
  }

  def hcatB(that: Doc) : Doc = {
    var thisLines =  Doc.padEnd(this.lines.reverse, that.height, " " * width).reverse
    thisLines = widen(thisLines)
    val thatLines = Doc.padEnd(that.lines.reverse, height, "").reverse
    Doc(thisLines.zip(thatLines).map(combine => combine._1 + combine._2))
  } 

  def vcat(that: Doc) : Doc = 
    Doc(this.lines ++ that.lines)
  ...
  {  
    jv match {
      ...
      case JNum(n) => 
        ...
        Doc(num)
      case JObj(o) => 
        val keys = o.keysIterator
        val lines = 
          for(i <- o.valuesIterator)
            yield{
              var nextKey = keys.next
              Doc(nextKey + " : ").hcatT(render(i))
            }
         val obj = lines.toList
         renderDocs(obj)
      case JArr(a) => 
        val arr = a.map(x => render(x))
        renderDocs(arr)
    }
...
object Json { 
  ...
  implicit def mapJson[A : Json] = new Json[Map[String, A]] { 
    def json(t: Map[String, A]): JVal = JObj(t.mapValues(x => Json.toJson(x)))
  }
