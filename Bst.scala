...
sealed abstract class BST[A <% Ordered[A]] extends AbstractSet[A] 
{
  ...
  { 
    ...
    {
      case Leaf() => false
      case Node(e, l, r) if x == e => true
      case Node(e, l, r) if x < e => l.contains(x)
      case Node(e, l, r) if x > e => r.contains(x)
    }
  }
 ...
 def fold[B](f: (B, A) => B, acc: B): B = 
  ...
  def add(x:A): BST[A] = 
  {
    this match
    {
      ...
      case Node(e, l, r) if x > e => Node(e, l, r.add(x))
      case Node(e, l, r) if x == e => this
  ...
  { 
    if(this.isEmpty)
      sys.error("Error: Can't Remove")

    var tree = this.getElts
    var min = tree(0)
  ...
  {
    this match{
      case Node(e, Leaf(), Leaf()) if x == e => Leaf()
      case Node(e, l, Leaf()) if x == e =>
        val left = l.removeMin
	Node(left._1, Leaf(), left._2)
      case Node(e, l , r) if x == e =>
        val right = r.removeMin
        Node(right._1 , l, right._2)
      case Node(e, l, r) if x > e => Node(e, l, r.remove(x))
...
object BST {
  ...
  def build[A <% Ordered[A]](xs: List[A]): BST[A] = {
    if (xs.length == 0) {
      ...
      var order = xs.sorted.distinct
      var mid = order(order.length/2)
      var left = order.take(order.length/2).filter(_ != mid )
      var right = order.takeRight(order.length/2).filter(_ != mid)
      Node(mid, build(left), build(right))
    }
  }
}
